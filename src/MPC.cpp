...
//Multipliers for the cost function. Tuning based on trial and error
int cte_cost = 2;
int eps_cost = 1;
int v_cost = 1;
int current_delta_cost = 2500;
int current_a_cost = 5;
int diff_delta_cost = 5000;
int diff_a_cost = 10;
 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 0; t < N; t++) {
      ...
      fg[0] += eps_cost*CppAD::pow(vars[epsi_start + t]-ref_epsi, 2);
...
//For example, if we're making a turn, we'd like the turn to be smooth, not sharp. Additionally, the vehicle velocity should not change too radically.
    for (int t = 0; t < N - 1; t++) {
      fg[0] += current_delta_cost*CppAD::pow(vars[delta_start + t], 2);
      fg[0] += current_a_cost*CppAD::pow(vars[a_start + t], 2);
    }
    ...
    for (int t = 0; t < N - 2; t++) {
      fg[0] += diff_delta_cost*CppAD::pow(vars[delta_start + t + 1] - vars[delta_start + t], 2);
      fg[0] += diff_a_cost*CppAD::pow(vars[a_start + t + 1] - vars[a_start + t], 2);
    }
...
double dt = 0.1;
 ...
 for (int i = 0; i < delta_start; i++) {
  ...
  // Acceleration and decceleration limits
  ...
  mpc_Yvals.clear();
  temp.clear();
  temp.push_back(solution.x[delta_start]);
  temp.push_back(solution.x[a_start]);
  for (int i = 0; i < N-1;i++){
    mpc_Xvals.push_back(solution.x[x_start+i]);
	mpc_Yvals.push_back((solution.x[y_start+i]));
