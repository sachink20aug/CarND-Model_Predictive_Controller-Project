    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          double py = j[1]["y"];
          double psi = j[1]["psi"];
          double v_read = j[1]["speed"]; //convert mph to mps
		  double v=v_read/2.2;
		...
		double dt=0.1;
		double x_new = px + v*cos(psi)*dt;
		double y_new = py + v*sin(psi)*dt;
		double psi_new = psi + (v * tan(-delta) / Lf) * dt + ((a * tan(-delta) / (2 * Lf)) * pow(dt, 2));
		double v_new = v + a * dt;
          ...
          for (int i = 0; i < ptsx.size(); i++) {
            double dtx = ptsx[i] - x_new;
            double dty = ptsy[i] - y_new;
            
            x_car_space[i] = dtx * cos(psi_new) + dty * sin(psi_new);
            y_car_space[i] = dty * cos(psi_new) - dtx * sin(psi_new);
          ...
          double psi_init = 0;
          state << x_pos, y_pos, psi_init, v_new, cte, epsi;
          
          // 5. Solve using MPC
          auto result = mpc.Solve(state, coeffs);
          
          // Steering angle from solver was reversed. 
         steer_value = -result[0]/deg2rad(25.0);
		throttle_value = result[1];
		 ...
		 mpc_x_vals  = mpc.mpc_Xvals;
		 mpc_y_vals =  mpc.mpc_Yvals;
